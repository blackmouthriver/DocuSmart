...
- ~~Tarjetas de favoritos con tamaños inconsistentes en el carrusel horizontal.~~ **Verificado en dispositivo real 2026-09-05, no reproducible** — `FavoriteDocumentCard` (`FavoritesSection.kt`) ya tiene tamaño fijo (`Modifier.width(150.dp).height(160.dp)`); probado con 3 favoritos de nombres corto/largo/truncado y las 3 tarjetas midieron igual. La nota original nunca se verificó en dispositivo — ver `docs/requirements/backlog-mejoras-ux-2026-08-30.md` §23.
...
- ~~Idioma: falta detección automática al primer inicio.~~ **Resuelto 2026-08-24** — `loadLanguage()` ahora usa el idioma del dispositivo como respaldo cuando no hay ninguno guardado (RF-SET-06). ~~Sigue pendiente ampliar el catálogo de idiomas (ja/ko/zh/it/fr).~~ **Resuelto 2026-09-04** — los 5 idiomas agregados, ver backlog §14.
...
- **Selector de archivo desde la biblioteca de la app implementado 2026-09-03** (item #15): Seguridad (Carpeta Segura, Contraseña PDF) y las 12 herramientas de un solo PDF de Herramientas PDF ganaron la opción de elegir un documento ya indexado por la app (`FileSourcePickerDialog` + `AppLibraryPickerViewModel`, nuevos en `core/ui/components`), no solo el selector del sistema. **Hallazgo real encontrado durante la verificación, no corregido (fuera de alcance de este ítem), ampliado a pedido del usuario 2026-09-03**: `DocumentRepository.loadPdfsFromDownloads()` devuelve 0 filas en el dispositivo real pese a que hay decenas de PDFs reales en `MediaStore.Downloads` -- todos con `owner_package_name=NULL` (no insertados vía `MediaStore.insert()` desde la app), lo que bajo scoped storage los hace invisibles a la consulta sin `READ_EXTERNAL_STORAGE` real. El usuario confirmó que el mismo síntoma ocurre con Excel/Word/Texto/PowerPoint desde el dispositivo real -- revisando el código son **dos problemas distintos**: (1) la misma limitación de scoped storage aplica a PDF/Word/Excel/PowerPoint por igual, ya que `loadPdfsFromDownloads()` los consulta juntos en un solo `mimeTypes` pese al nombre de la función; (2) **Texto es un bug aparte y más simple** -- `"text/plain"` nunca se incluyó en la lista `mimeTypes` de esa consulta, así que un `.txt`/`.md` de Descargas ni siquiera se evalúa, sin importar el propietario. Confirmado que el selector nuevo en sí funciona correctamente probándolo sin el filtro de PDF (con Imágenes, que sí carga bien vía `READ_MEDIA_IMAGES`). Probablemente afecta también el conteo de las categorías "PDF"/"Word"/"Excel"/"PowerPoint"/"Texto" en la Biblioteca real -- catalogado como fila 22 del backlog UX para investigar y corregir aparte. Verificado en dispositivo real: gauntlet en verde, `connectedDebugAndroidTest` de Seguridad/Herramientas PDF (3/3), flujo completo de punta a punta (Carpeta Segura → elegir de biblioteca → proteger). Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §15.
- **Bug de Descargas corregido lo corregible, 2026-09-03** (fila 22 del backlog UX): confirmado por búsqueda externa antes de tocar nada que `android.permission.READ_MEDIA_DOCUMENTS` (ya declarado en el manifest con el comentario "para leer PDF en Android 13+") **no existe como permiso real de Android** -- un intento de arreglo de una sesión anterior no tenía ningún efecto. También confirmado: desde API 33, `READ_EXTERNAL_STORAGE` ya no tiene ningún efecto, y no existe ningún permiso equivalente a `READ_MEDIA_IMAGES` para documentos de otras apps -- solo SAF (`ACTION_OPEN_DOCUMENT`, ya usado) o `MANAGE_EXTERNAL_STORAGE` (no solicitado). Se corrigió lo que sí es corregible: permiso falso eliminado del manifest; `READ_EXTERNAL_STORAGE` amplió su `maxSdkVersion` de 28 a 32 (rango donde sí tiene efecto real); `DocumentRepository.loadPdfsFromDownloads()` renombrada a `loadDocumentsFromDownloads()` (el nombre ocultaba que ya consultaba Word/Excel/PowerPoint también) y ganó `"text/plain"`/`"text/markdown"` en su filtro de `mimeTypes` (antes ni estaban, bug independiente del de scoped storage). **Lo que sigue sin arreglo posible**: en API 33+ (el dispositivo real de esta sesión incluido) ningún código puede hacer visibles documentos de Descargas que la app no creó -- restricción real de la plataforma, no un bug. Verificado en dispositivo real: gauntlet completo en verde (detekt necesitó actualizar 1 línea de baseline por el rename), `connectedDebugAndroidTest` de Biblioteca/Seguridad/Herramientas PDF (9/9). Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §16.
- **Vincular carpeta de Descargas por SAF implementado y verificado en dispositivo real, 2026-09-03** (alternativa real a la limitación de la fila 22/§16, pedida explícitamente por el usuario): `DownloadsAccessManager` nuevo envuelve `ACTION_OPEN_DOCUMENT_TREE` + permiso persistente; `DocumentRepository` enumera la carpeta vinculada vía `DocumentFile` (sin la restricción de "solo filas propias" de scoped storage) y borra documentos SAF vía `DocumentsContract.deleteDocument()`; tarjeta "Vincular carpeta" en Biblioteca (pestaña Dispositivo) y gestión vincular/desvincular en Ajustes → Almacenamiento. **Hallazgo real encontrado en la propia verificación**: Android (confirmado en el Motorola Edge 30 Neo, API 34, y por búsqueda externa) **no permite vincular la carpeta "Descargas" en sí misma ni la raíz del almacenamiento** vía SAF -- muestra "No se puede usar esta carpeta" con el botón de selección deshabilitado; solo una *subcarpeta* dentro de Descargas es seleccionable (verificado vinculando `Descargas/DMSS` con éxito de punta a punta: permiso persistido, Biblioteca/Ajustes reflejan el estado correctamente, desvincular funciona). En la práctica la función no resuelve el caso más común (un PDF que el navegador guardó directo en la raíz de Descargas) sin que el usuario mueva antes sus archivos a una subcarpeta. Verificado en dispositivo real: gauntlet completo en verde (detekt necesitó subir `TooManyFunctions.thresholdInClasses` de 15 a 20 y sumar 2 entradas de baseline, mismo criterio ya usado en el proyecto; 2 tests de `DocumentRepositoryTest` ajustados para stubear `Uri.authority`). Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §17.
- **Refinamientos a la vinculación de Descargas pedidos por el usuario tras la primera verificación, 2026-09-03**: (1) texto del banner reemplazado por copia propia del usuario ("Elije tu carpeta preferida para encontrar tus documentos de tu dispositivo y vincularlo a DocuSmart"), sin la explicación técnica -- decisión consciente, el propio Android igual bloquea Descargas con su aviso de sistema si el usuario lo intenta; (2) nuevo botón compacto de atajo junto a Dispositivo/Mis archivos/Papelera (visible solo con carpeta vinculada) que abre la carpeta real vía `Intent(ACTION_VIEW)` + `DocumentsContract.Document.MIME_TYPE_DIR`, delegando en el gestor de archivos del sistema; la etiqueta de las 4 tarjetas bajó de `labelLarge` a `labelMedium` para que "Dispositivo" no se partiera en dos líneas al angostarse por la 4ª columna. **Hallazgo menor**: en el Motorola Edge 30 Neo, la app Archivos de Google abre su vista de "Descargas" (carpeta padre) en vez de saltar directo a la subcarpeta vinculada -- no hay API estándar de Android para forzar el salto exacto, varía por gestor de archivos. El usuario también preguntó por qué las imágenes sí se ven del dispositivo completo pero los documentos no: respondido que es una asimetría real y deliberada de Android 13+ (`READ_MEDIA_IMAGES`/`READ_MEDIA_VIDEO` sí existen y son de amplio alcance; nunca se creó un permiso equivalente para documentos, que quedaron bajo la regla general de scoped storage). Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §17.
- **Grilla 2x2 + nombre real de carpeta + onboarding con vincular carpeta, 2026-09-03**: (1) con carpeta vinculada, Dispositivo/Mis archivos/Papelera/Carpeta pasan de una fila de 4 (angosta) a una grilla 2x2; el atajo muestra el nombre real de la carpeta (`DocumentFile.fromTreeUri(uri).name`, ej. "DMSS") en vez de un genérico "Carpeta". (2) Nueva 5ª slide en el onboarding (`OnboardingScreen.kt` + `OnboardingViewModel` nuevo, mismo patrón que `LibraryViewModel`/`SettingsViewModel`) que deja vincular una carpeta desde el primer arranque, con estado reactivo ("Vinculada: <nombre>" + "Cambiar carpeta", o "Vincular carpeta" si no hay ninguna) -- no bloquea el flujo, el usuario puede saltarlo. Motivado por la pregunta del usuario de si sin vincular/elegir archivos el dispositivo completo podía verse igual: confirmado que no, es una regla de Android 13+ para toda app externa no-gestora-de-archivos. **`MANAGE_EXTERNAL_STORAGE` evaluado y descartado como alternativa**: el criterio de revisión de Google Play es técnico ("solo pedirlo si SAF/MediaStore no alcanzan"), no de calidad del consentimiento -- como SAF ya funciona, pedir ese permiso sería evidencia en contra, y el castigo por incumplir la política es la remoción completa de la app de Play Store. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §17.
- **Visor de PowerPoint reescrito con Apache POI, 2026-09-03** (§18): el usuario pidió mejorar el renderizado de Word/Excel/PowerPoint (señaló, correctamente, que no se veían como el archivo original -- confirmado leyendo el código: extracción de texto crudo por regex, no un renderizador real). Tras explicar honestamente que un renderizador OOXML propio desde cero es un proyecto de años y que "mostrarlo como imagen" no evita el problema (alguien igual tiene que decidir qué dibujar y dónde), el usuario eligió el camino intermedio: Apache POI (ya usado en producción por `WordToTextUseCase`/`WordToPdfUseCase`/`ExcelToPdfUseCase`, compatibilidad con Android ya probada) + renderer propio en Compose. PowerPoint reescrito primero (peor estado de los tres): `XMLSlideShow`/`XSLFShape` para texto real con formato + imágenes reales, párrafos separados con salto de línea (antes se pegaban: "Punto unoPunto dos"). **Hallazgo real**: `XSLFShape.anchor`/`XMLSlideShow.pageSize` (tipos `java.awt.geom.Rectangle2D`/`Dimension`) **ni compilan** contra el SDK de Android -- se descartó el posicionamiento exacto de cada forma, quedan apiladas en orden original. **Bug real encontrado y corregido**: `ClassNotFoundException: com.zaxxer.sparsebits.SparseBitSet` -- crash reproducido en dispositivo real al abrir cualquier .pptx, causado por un `exclude(group = "com.zaxxer")` de una sesión anterior (válido para Word/Excel, no para PowerPoint); corregido agregando `com.zaxxer:SparseBitSet:1.3` explícito. Verificado sin crash en dispositivo real con un .pptx real. **Pendiente cosmético, no bloqueante**: el color del cuerpo se ve más azul de lo esperado en vez de gris oscuro. **Word y Excel quedan pendientes** para una siguiente sesión (mismo enfoque POI). Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §18.
- **Bug de compartir + Biblioteca con historial permanente, 2026-09-03** (§19): el usuario reportó que compartir un Word generado por la app (conversión desde WhatsApp) ya no funcionaba, y que la carpeta vinculada por SAF seguía sin mostrar archivos -- al punto de preguntar si abandonar el enfoque SAF-only. **2 bugs reales encontrados y corregidos**: (1) `ViewerViewModel.shareDocument()` exponía un `file://` real a otras apps (`FileUriExposedException` desde API 24, atrapada como error genérico) -- Biblioteca/Home ya usaban `FileProvider` para esto pero el Visor nunca lo hizo; corregido y verificado con WhatsApp disponible en el selector. (2) `loadDocumentsFromLinkedFolder()` no recorría subcarpetas -- el usuario había vinculado "Documents" (vacía salvo una subcarpeta ajena), de ahí que no se viera nada; corregido con recorrido recursivo (`collectLinkedFolderDocuments`, tope de profundidad 8). **Ampliación real de Biblioteca**: se agregó `DocumentHistoryDao.allEntries()` + `DocumentRepository.loadDocumentsFromHistory()` -- cualquier documento abierto alguna vez (vía "Abrir con DocuSmart" desde otra app, o el selector) queda visible en Biblioteca de forma permanente, no solo mientras esté entre los 5 recientes de Inicio; de paso se corrigió `isDeviceDocument()` (clasificaba mal documentos de proveedores externos como WhatsApp/Gmail como "Mis archivos"). **Investigado sin cambios de código**: confirmado por git log que `targetSdkVersion` fue 35/36 en toda la historia del proyecto -- la app nunca pudo mostrar documentos ajenos sin acción explícita, lo que el usuario recuerda de "versiones pasadas" son probablemente PDFs/imágenes generados por la propia app. También confirmado que "activar permisos de carpetas desde Ajustes del sistema" (como hacen otras apps) es literalmente `MANAGE_EXTERNAL_STORAGE` visto desde otra puerta -- mismo riesgo de política ya evaluado en §17; el usuario decidió mantener el enfoque solo-SAF. Verificado en dispositivo real de punta a punta. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §19.
- **Visor de Word y Excel reescritos con Apache POI, 2026-09-03** (mismo día, §19): tras fusionar el trabajo anterior, el usuario propuso 2 alternativas de terceros (visor de Google vía WebView; API de conversión en la nube Cloudmersive) para mejorar el renderizado -- evaluadas y descartadas: ambas exponen documentos privados del usuario a terceros (una necesita URL pública, la otra sube el archivo a una API externa), contradiciendo la promesa de privacidad de la app, y verificado con búsqueda externa que el plan gratis real de Cloudmersive es 600/mes con tope de 2.5 MB por archivo (no 800 como se había leído). Se confirmó seguir con Apache POI. **Word**: `XWPFDocument` iterando `bodyElements` (preserva el orden real párrafo/tabla, a diferencia de leer `paragraphs`+`tables` por separado) -- reutiliza `detectWordFormat()`/`extractLegacyDocBlocks()`/`isHeadingStyleName()` del converter en vez de duplicar esa lógica; las tablas ahora se ven como grilla real, no como texto plano intercalado. **Excel**: `WorkbookFactory` (detecta `.xls`/`.xlsx` solo) + `DataFormatter`+`FormulaEvaluator` (fechas/monedas/fórmulas formateados como Excel los muestra) + pestañas para múltiples hojas (antes solo se leía `sheet1.xml`, hardcodeado). `WordRunParsingTest.kt` (obsoleto, probaba la función regex eliminada) reemplazado por `WordViewerExtractionTest.kt`, con `.docx` reales construidos en memoria vía la propia API de escritura de POI. **Hallazgo real durante la verificación**: `pruebaword.docx` (la conversión de WhatsApp que reportó el usuario) se sigue viendo sin espacios entre palabras -- confirmado que es un bug del conversor PDF/imagen→Word que generó ese archivo específico, no del visor nuevo (un `.docx` no convertido se ve con espaciado perfecto). Verificado en dispositivo real: negrita/cursiva/tamaño/tablas en Word, y una hoja de cálculo real con encabezado y datos en Excel, todo correcto. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §19.
- **Bug del conversor PDF→Word corregido, 2026-09-03** (mismo día, §19): investigado a pedido del usuario el hallazgo anterior de `pruebaword.docx` sin espacios entre palabras. Causa raíz: `PdfToWordUseCase` (RF-CONV-09) solo insertaba un espacio al detectar un salto de línea real dentro del mismo párrafo -- muchos generadores de PDF (el que produjo el PDF original de WhatsApp incluido) codifican el espacio entre palabras de la MISMA línea como un simple desplazamiento horizontal del cursor, sin ningún carácter `" "` real, algo que la lógica no contemplaba porque `TextChunk` nunca registraba coordenadas X. Corregido agregando `xStart`/`xEnd` a `TextChunk` (vía `TextRenderInfo.baseline`) y una nueva regla: si dos fragmentos comparten línea y el hueco horizontal entre ellos supera `WORD_GAP_MULTIPLIER` (0.2x el tamaño de fuente, empírico), se inserta un espacio. `buildDocx()` se refactorizó extrayendo `classifyChunkPlacement()` para no exceder el límite de complejidad ciclomática de detekt. Test nuevo en `PdfToWordUseCaseTest.kt` reproduce el caso exacto (dos fragmentos en la misma línea, 100pt de separación, sin espacio literal) y verifica el espacio insertado. Gauntlet completo (`compileDebugKotlin`, `detekt`, `lintDebug`, `testDebugUnitTest`) en verde. **Verificado en dispositivo real** (Motorola Edge 30 Neo): reconvertido el PDF real de WhatsApp (`VACACIONES ADILA.pdf`) desde el flujo normal de la app -- el `.docx` resultante ahora muestra "Funza, Cundinamarca, 03 de septiembres de 2026" con espacios correctos, confirmado inspeccionando el XML crudo extraído del almacenamiento de la app. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §19.
- **Bug "imagen dentro del título Estudio" reproducido y corregido, 2026-09-03** (§10, ítem #8): navegando en el dispositivo real a Estudio → Pomodoro se confirmó que el `TopAppBar` es texto puro, pero el chip indicador de tipo de sesión (`PomodoroTypeIndicator`) mostraba "📚 Estudio"/"🌿 Descanso" -- un emoji pegado al texto en los 5 idiomas (`study_study_label`/`study_break_label`). El usuario confirmó que era esto y pidió eliminar cualquier emoji en títulos de la app. Búsqueda en los 5 `strings.xml` encontró un segundo caso confirmado por el usuario: `premium_recommended` ("⭐ Recomendado", badge de la tarjeta de plan recomendado en Premium). Ambos strings corregidos a texto plano en los 5 idiomas (es/de/en/pt/ru); los únicos símbolos restantes en rango de emoji son checkmarks "✓" de confirmación y flechas "→" tipográficas, ninguno en un título -- fuera de alcance. Verificado en dispositivo real: chip de Pomodoro ahora dice solo "Estudio", badge de Premium solo "Recomendado". Gauntlet completo en verde. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §10.
- **Bug "Ahorra 44%%" corregido, 2026-09-03** (mismo día, §10): hallazgo detectado al verificar el fix anterior. `premium_savings_44` ("Ahorra 44%%") y `premium_feature_compress_desc` ("Reduce PDFs hasta un 90%%...") se leen con `stringResource()` plano (sin argumentos de formato) en `PremiumPlanCards.kt`/`PremiumFeatureList.kt` -- el `%%` de escape solo se colapsa a un `%` cuando el string pasa por `String.format`/`getString(id, args)`, que no es el caso acá, así que se mostraba el `%` duplicado literal. Corregidos a un solo `%` en los 5 idiomas, con `tools:ignore="StringFormatInvalid"` puntual (lint marca cualquier `%` suelto como potencial format string). Se descartó tocar `pdf_compress_success`/`pdf_crop_success` (mismo patrón a primera vista) porque esos sí se formatean después con argumentos reales -- su `%%` es correcto. Verificado en dispositivo real: "Reduce PDFs hasta un 90%" y "Ahorra 44%" con un solo signo. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §10.
- **Auditoría honesta del plan Premium, 2026-09-03** (mismo día, §10): el usuario preguntó si "Nube integrada" y las limitantes del visor debían seguir prometidas en Premium dado que no se han construido. Se auditaron los 8 beneficios listados contra el código real, no solo ese: **"PDF a Excel" y "PDF a PowerPoint" resultaron ser beneficios completamente inexistentes** (el proyecto solo tiene las conversiones en la dirección contraria, `ExcelToPdfUseCase`/`PptToPdfUseCase` -- ningún `PdfToExcelUseCase`/`PdfToPptUseCase`), y **"OCR avanzado: 50+ idiomas" resultó exagerado** (el OCR real usa `com.google.mlkit:text-recognition`, el reconocedor solo de alfabeto latino, sin los módulos para chino/japonés/coreano/devanagari). "Nube integrada" (Drive/Dropbox) confirmado sin ninguna lógica real, solo texto de marketing. El usuario eligió eliminar las 3 promesas falsas/exageradas en vez de dejarlas o marcarlas "próximamente", dado el riesgo real (no solo de UX) de vender una suscripción con beneficios inventados. Corregido: `PDF_TO_EXCEL`/`PDF_TO_PPT`/`CLOUD_SYNC` eliminados del enum `PremiumFeature` (`PremiumPlan.kt`) y su mapeo de ícono (`PremiumFeatureList.kt`) -- la lista se renderiza sola vía `PremiumFeature.values()`; sus 6 strings eliminados de los 5 idiomas; `premium_feature_ocr_desc` reescrito sin la cifra de idiomas, describiendo lo que el OCR real hace. Verificado en dispositivo real: Premium ahora solo lista 5 beneficios (sin anuncios, PDF a Word, OCR, compresión 90%, conversiones ilimitadas), todos confirmados reales. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §10.
- **Firebase Analytics/Crashlytics arreglados de verdad, 2026-09-03** (§20, fila 23): el usuario pidió sumar Firebase para analítica, revisión de errores y monetización. Investigado antes de agregar nada: **ya existía una integración de una sesión anterior, pero nunca funcionó** -- `google-services.json` y las dependencias de Firebase estaban, pero los plugins de Gradle que procesan ese archivo (`com.google.gms.google-services`/`com.google.firebase.crashlytics`) nunca se aplicaban en `app/build.gradle.kts`; los 15 eventos de `DocuSmartAnalytics.kt` tenían cero call sites en toda la app; y en build `release` no había ningún árbol de Timber plantado, así que ni los `Timber.e`/`Timber.w` ya existentes llegaban a ningún lado. Más grave: `deployment.md` (la declaración de Seguridad de los datos de Play Store) ya afirmaba que la app envía estos datos -- una promesa incumplida. Corregido: plugins aplicados, meta-data del manifest conectado, los 15 eventos conectados a sus flujos reales (conversión, herramientas PDF, documentos, favoritos, escaneo, QR, Pomodoro, notas, Premium -- `logScreenView` centralizado en un único listener de navegación), y un `CrashlyticsTree` (Timber) que reenvía cualquier `Timber.w`/`Timber.e` con excepción real hacia Crashlytics como no fatal. En el camino: `assembleRelease` nunca se había compilado exitosamente con Firebase -- 2 bugs reales encontrados y corregidos (R8 fallaba por `java.awt.*` de una dependencia transitiva de POI; el plugin de Crashlytics 2.9.9 fallaba con Gradle 9.5 por una dependencia de Groovy, resuelto subiendo a 3.0.6). **Verificado en dispositivo real con un build release firmado**: logcat confirmó a Firebase inicializando por primera vez en la historia del proyecto, eventos `screen_view`/`pdf_tool_used` reales encolándose, y una solicitud HTTPS real al endpoint de Firebase. Test Lab y Remote Config (monetización) quedan para una siguiente sesión, a pedido del usuario. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §20.
- **i18n: 5 idiomas nuevos (ja/ko/zh/it/fr), 2026-09-04** (§21, fila 14): la app ya soportaba es/en/de/pt/ru; se agregaron japonés, coreano, chino, italiano y francés al enum `AppLanguage` (`LanguageManager.kt`). Dado el volumen (724 strings × 5 idiomas), la traducción se delegó a 5 agentes en paralelo, cada uno verificando su propio archivo (conteo de 724 entradas, mismos `name` en el mismo orden que el fuente, XML válido, especificadores de formato preservados, "DocuSmart"/"mouthblack technology" sin traducir). Verificación independiente después confirmó los 5 archivos correctos. **Bug de test encontrado y corregido**: `LanguageManagerTest.kt` usaba japonés como ejemplo de idioma NO soportado en 2 tests -- al agregarlo de verdad, esos tests correctamente empezaron a fallar; cambiados a árabe (que sigue sin soporte), mismo ajuste reflejado en `settings-premium.md` AC2. **Bug real de UI encontrado y corregido en la verificación en dispositivo**: el diálogo "Seleccionar idioma" no tenía scroll -- con 10 idiomas, la última fila (Francés) quedaba con texto colapsado a altura cero, invisible e inseleccionable (confirmado con volcado de accesibilidad: el `RadioButton` se dibujaba pero su `Text` estaba vacío). Corregido agregando `verticalScroll` al diálogo. Verificado en dispositivo real: los 10 idiomas visibles con scroll, cambio a japonés confirmado con toda la UI traducida correctamente, vuelta a español. Gauntlet completo en verde. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §21.
- **Monetización: Firebase Remote Config + Firebase Test Lab en CI, 2026-09-04** (§22): segunda mitad de lo pedido junto con Analytics/Crashlytics (§20), retomada a pedido del usuario. **Remote Config**: `RemoteConfigManager` (nuevo) con 2 parámetros (`premium_annual_highlighted`, `premium_show_savings_badge`), valores por defecto iguales al comportamiento previo, `PremiumRepository.getAvailablePlans()` los usa para decidir qué plan se destaca y si se muestra el badge de ahorro -- ajustable desde la consola de Firebase sin publicar actualización. A propósito NO se hizo dinámico el precio mostrado (lo cobra Play Billing, no Remote Config -- desincronizarlos sería engañoso). Test unitario nuevo (`PremiumRepositoryTest.kt`, 3 casos). Verificado en dispositivo real: sin nada configurado en la consola, la pantalla Premium se ve idéntica a antes, sin errores. **Firebase Test Lab**: nuevo workflow `firebase-test-lab.yml`, disparo manual (`workflow_dispatch`). Motivación real: el job `instrumented-tests` de `ci.yml` es `continue-on-error: true` desde 2026-09-02 porque 14-15/30 pruebas fallan solo en el emulador de GitHub Actions, no en dispositivo real -- Test Lab corre las mismas pruebas en dispositivos de Google en vez de ese emulador. El job está gateado con `if: secrets.GCP_SA_KEY != ''`, así que se puede fusionar ya mismo sin romper nada. **Corrección 2026-09-04**: el usuario preguntó por el costo (no puede pagar un plan de pago por ahora) -- investigado y confirmado que la primera versión de esta documentación estaba **equivocada**: Test Lab no requiere el plan Blaze, el plan Spark (gratis, sin tarjeta) ya incluye una cuota real de hasta 15 pruebas/día (10 virtuales + 5 físicas); vincular una cuenta de facturación es lo único que subiría el proyecto a Blaze, así que basta con no hacerlo. **Pendiente del usuario, son pasos de consola web que no requieren tarjeta ni plan de pago**: crear una cuenta de servicio de GCP con rol "Firebase Test Lab Admin", generar su clave JSON y guardarla como secret `GCP_SA_KEY` en GitHub -- detalle paso a paso en el backlog. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §22.
- **HU-UX-06 retomada y acotada a un bug real, 2026-09-04** (§7, fila 5): antes de diseñar la épica completa de colores independientes por banner/íconos/cards/nav bar, se investigó el sistema de theming actual para fundamentar la pregunta de diseño pendiente -- hallazgo real: `HomeBanner.kt` tenía su degradado y el texto del botón "Abrir" fijos en tonos de azul (`DocuBlue`/`SmartBlue`/`IndigoAccent`), ignorando por completo el "Color de acento" que el usuario ya puede elegir en Ajustes; era el único elemento de Home que no lo respetaba (la barra de navegación inferior sí, vía defaults de Material3; los íconos de Accesos rápidos usan colores fijos por diseño intencional, no relacionado). Presentado el hallazgo con 3 opciones de alcance, el usuario eligió arreglar solo el bug, no la épica completa. Corregido derivando el degradado y el color del botón de `MaterialTheme.colorScheme.primary` (`lerp` hacia blanco/negro para reproducir el mismo efecto de 3 tonos) en vez de constantes fijas. Verificado en dispositivo real: con acento "Rosa" el banner y el botón cambian a rosa; de vuelta en "Azul" (el default) se ve idéntico al diseño original, sin regresión. HU-UX-06 completa (colores independientes por zona) queda descartada por ahora. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §7.
- **Fix del banner extendido a todos los banners azules de la app, 2026-09-04** (mismo día, §7): el usuario pidió que el fix de Home aplicara a "todos los banners azules", no solo ese. Se encontraron 6 lugares más con el mismo bug exacto: `DocuSmartTopBanner.kt` (componente compartido por 9 pantallas -- Ajustes, Seguridad, Herramientas PDF, etc.), `PremiumBanner.kt`, el fondo de carga de `ScannerScreen.kt`, 2 fondos de pantalla completa en `SecurityScreen.kt` (menú principal + "Configura tu PIN", esta última también con su botón de texto azul fijo), y el ícono decorativo + gráfico de barras de `StudyScreen.kt`. Extraída la lógica a un helper compartido `rememberAccentGradient()` (nuevo, `core/ui/theme/AccentGradient.kt`) para no duplicarla. **Hallazgo adicional**: `DocuSmartCards.kt` tenía una función `DocuSmartGradientCard` con el mismo bug pero **sin ningún call site en toda la app** (código muerto, `HomeBanner.kt` ya tiene su propia implementación) -- eliminada en vez de corregida. Dejado a propósito sin tocar: los indicadores semánticos de estado en `StudyScreen.kt` (TTS activo, Pomodoro estudio/descanso -- codifican un estado, no son decorativos), y Splash/Onboarding (identidad de marca antes de que el usuario explore Ajustes). Verificado en dispositivo real con acento "Verde": Ajustes, Seguridad (menú + pantalla de PIN) y Premium cambian correctamente; de vuelta en "Azul" sin regresión. Gauntlet completo en verde. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §7.

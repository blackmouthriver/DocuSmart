...
- **Banner de anuncios ampliado a 3 pantallas más, 2026-08-31** (bucket 2, HU-UX-07): Resultado de escaneo (reusa `converterViewModel.adManager`), Menú de Seguridad (se creó un `SecurityMenuViewModel` mínimo, no tenía ninguno) y el Visor (banner agrupado con la barra de controles inferior, se oculta/muestra junto con ella en vez de tapar el documento fijo). Confirmado con el usuario: Contraseña PDF/Carpeta Segura/Papelera quedan sin anuncios (eran pantallas de acción rápida/seguridad). Pendiente de confirmación del usuario: Estudio, Lector QR y Creador QR no tienen ningún ViewModel hoy — agregarles uno solo para esto es más trabajo del estimado originalmente, y Escáner (pantalla transitoria) se propone excluir del todo. Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §8.

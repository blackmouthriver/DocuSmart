...
- **Bug del conversor PDF→Word corregido, 2026-09-03** (mismo día, §19): investigado a pedido del usuario el hallazgo anterior de `pruebaword.docx` sin espacios entre palabras. Causa raíz: `PdfToWordUseCase` (RF-CONV-09) solo insertaba un espacio al detectar un salto de línea real dentro del mismo párrafo -- muchos generadores de PDF (el que produjo el PDF original de WhatsApp incluido) codifican el espacio entre palabras de la MISMA línea como un simple desplazamiento horizontal del cursor, sin ningún carácter `" "` real, algo que la lógica no contemplaba porque `TextChunk` nunca registraba coordenadas X. Corregido agregando `xStart`/`xEnd` a `TextChunk` (vía `TextRenderInfo.baseline`) y una nueva regla: si dos fragmentos comparten línea y el hueco horizontal entre ellos supera `WORD_GAP_MULTIPLIER` (0.2x el tamaño de fuente, empírico), se inserta un espacio. `buildDocx()` se refactorizó extrayendo `classifyChunkPlacement()` para no exceder el límite de complejidad ciclomática de detekt. Test nuevo en `PdfToWordUseCaseTest.kt` reproduce el caso exacto (dos fragmentos en la misma línea, 100pt de separación, sin espacio literal) y verifica el espacio insertado. Gauntlet completo (`compileDebugKotlin`, `detekt`, `lintDebug`, `testDebugUnitTest`) en verde. Pendiente verificación en dispositivo real (ningún dispositivo conectado por adb al cerrar esta sesión). Ver [`docs/requirements/backlog-mejoras-ux-2026-08-30.md`](docs/requirements/backlog-mejoras-ux-2026-08-30.md) §19.

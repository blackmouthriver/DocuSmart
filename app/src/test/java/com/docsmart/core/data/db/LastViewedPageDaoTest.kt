...
class LastViewedPageDaoTest {
    ...
    @Test
    fun `getByDocument devuelve null si nunca se guardo nada`() =
        runTest {
            assertNull(dao.getByDocument("doc-1"))
        }

    @Test
    fun `save guarda la pagina y getByDocument la devuelve`() =
        runTest {
    ...
    @Test
    fun `save es upsert -- volver a guardar el mismo documento actualiza la pagina, no duplica`() =
        runTest {
    ...
    @Test
    fun `deleteByDocument borra el registro`() =
        runTest {
            dao.save(LastViewedPageEntity("doc-1", page = 3, updatedAt = 1000L))
    ...
    @Test
    fun `updateDocumentId migra la fila al id nuevo`() =
        runTest {

...
class TrashDaoTest {
    ...
    @Test
    fun `insert agrega una entrada nueva`() =
        runTest {
    ...
    @Test
    fun `insert es upsert - volver a eliminar el mismo documento actualiza la fecha, no duplica`() =
        runTest {
            dao.insert(TrashEntry("doc-1", 1000L))
    ...
    @Test
    fun `remove elimina la entrada y ya no aparece en getAll`() =
        runTest {
            dao.insert(TrashEntry("doc-1", 1000L))

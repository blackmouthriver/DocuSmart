...
class AnnotationDaoTest {
    ...
    private fun highlight(id: String, documentId: String = "doc-1", page: Int = 1) = AnnotationEntity(
        id = id, documentId = documentId, type = AnnotationType.HIGHLIGHT, page = page,
        xPts = 10f, yPts = 20f, widthPts = 100f, heightPts = 15f,
        color = 0xFFFFEB3B.toInt(), text = "", createdAt = 1000L
    )

    private fun note(
        id: String, documentId: String = "doc-1", page: Int = 1, text: String = "Recordar esto"
    ) = AnnotationEntity(
        id = id, documentId = documentId, type = AnnotationType.NOTE, page = page,
        xPts = 30f, yPts = 40f, widthPts = 0f, heightPts = 0f,
        color = 0xFFFF7043.toInt(), text = text, createdAt = 2000L
    )

    @Test
    fun `insert agrega una anotacion nueva`() = runTest {
    ...
    @Test
    fun `getByDocument no devuelve anotaciones de otro documento`() = runTest {
    ...
    @Test
    fun `insert conserva texto y color reales de una nota`() = runTest {
    ...
    @Test
    fun `delete elimina solo la anotacion indicada`() = runTest {
        dao.insert(highlight("a1"))
        dao.insert(note("n1"))
    ...
    @Test
    fun `delete de un id que no existe no falla`() = runTest {
    ...
    @Test
    fun `observeByDocument refleja un insert posterior en una suscripcion nueva`() = runTest {
    ...
    // suscribirse.
    @Test
    fun `observeByDocument emite una fila nueva a un colector ya activo, sin resuscribirse`() = runTest {
    ...
    @Test
    fun `insert con el mismo id reemplaza en vez de duplicar`() = runTest {
        dao.insert(highlight("a1"))
    ...
    @Test
    fun `una anotacion recien creada no existe hasta insertarla`() = runTest {

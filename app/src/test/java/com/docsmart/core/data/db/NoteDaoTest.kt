...
class NoteDaoTest {
    ...
    private fun note(id: String, title: String = "Título", documentId: String? = null, reminderAt: Long? = null) =
        NoteEntity(
            id = id, title = title, text = "Contenido de $id", createdAt = id.hashCode().toLong(),
            documentId = documentId, reminderAt = reminderAt
        )

    @Test
    fun `insert agrega una nota nueva`() = runTest {
        dao.insert(note("n1"))
    ...
    @Test
    fun `observeAll ordena de mas reciente a mas antiguo`() = runTest {
    ...
    @Test
    fun `insertImage asocia la imagen a la nota via el Relation`() = runTest {
        dao.insert(note("n1"))
    ...
    @Test
    fun `delete de una nota borra tambien sus filas de imagen via CASCADE`() = runTest {
        dao.insert(note("n1"))
        dao.insertImage(NoteImageEntity(noteId = "n1", filePath = "/a.jpg", position = 0))
        ...
        dao.insert(note("n1"))
        assertTrue(dao.observeAll().first().first().images.isEmpty())
    }

    @Test
    fun `deleteAll borra todas las notas`() = runTest {
        dao.insert(note("n1"))
        dao.insert(note("n2"))
    ...
    @Test
    fun `observeByDocument solo devuelve las notas vinculadas a ese documento`() = runTest {
    ...
    @Test
    fun `observeLinkedCount cuenta las notas vinculadas a un documento`() = runTest {
        dao.insert(note("n1", documentId = "doc-1"))
    ...
    @Test
    fun `updateDocumentId migra el vinculo al id nuevo`() = runTest {
    ...
    @Test
    fun `unlinkDocument desvincula sin borrar la nota (AC2)`() = runTest {
        dao.insert(note("n1", documentId = "doc-1"))

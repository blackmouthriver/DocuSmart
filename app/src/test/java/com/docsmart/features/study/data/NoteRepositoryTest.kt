...
class NoteRepositoryTest {
    ...
    @Test
    fun `updateNote actualiza titulo, texto y recordatorio`() = runTest {
        repository.createNote("Original", "texto original")
        val note = db.noteDao().observeAll().first().first().note

        repository.updateNote(
            noteId = note.id, title = "Editado", text = "texto editado", reminderAt = 5_000L,
            keptImages = emptyList(), removedImages = emptyList(), newImageUris = emptyList()
        )

        val updated = db.noteDao().observeAll().first().first().note
    ...
    @Test
    fun `updateNote conserva las imagenes mantenidas, borra del disco las removidas y agrega las nuevas`() = runTest {
        ...
        val original = db.noteDao().observeAll().first().first()
        ...
        repository.updateNote(
            noteId = original.note.id, title = original.note.title, text = original.note.text, reminderAt = null,
            keptImages = keep, removedImages = remove, newImageUris = listOf(uriC)
        )

        val result = db.noteDao().observeAll().first().first()
    ...
    @Test
    fun `updateNote cancela el recordatorio viejo y programa el nuevo cuando cambia`() = runTest {
        repository.createNote("Con recordatorio", "texto", reminderAt = 1_000L)
        val note = db.noteDao().observeAll().first().first().note

        repository.updateNote(
            noteId = note.id, title = note.title, text = note.text, reminderAt = 2_000L,
            keptImages = emptyList(), removedImages = emptyList(), newImageUris = emptyList()
        )
    ...
    @Test
    fun `updateNote no toca el scheduler si nunca hubo recordatorio ni se agrega uno`() = runTest {
        repository.createNote("Sin recordatorio", "texto")
        val note = db.noteDao().observeAll().first().first().note

        repository.updateNote(
            noteId = note.id, title = note.title, text = "texto editado", reminderAt = null,
            keptImages = emptyList(), removedImages = emptyList(), newImageUris = emptyList()
        )
    ...
    // imagen.
    @Test
    fun `linkDocument no borra las imagenes ya adjuntas a la nota`() = runTest {
        val uri = stubImageUri(byteArrayOf(1))
        repository.createNote("Con imagen", "texto", imageUris = listOf(uri))
        val note = db.noteDao().observeAll().first().first().note
        assertEquals(1, db.noteDao().observeAll().first().first().images.size)

...
class AgendaRepositoryTest {
    ...
    @Test
    fun `createEvent guarda el evento y programa el recordatorio`() = runTest {
        val saved = slot<AgendaEventEntity>()
        coEvery { dao.insert(capture(saved)) } just Runs

        val result = repository.createEvent(
            title = "Reunión", description = "Con el equipo", dateTimeMillis = 5_000L,
            documentId = "doc-1", reminderMinutesBefore = 60
        )
    ...
    @Test
    fun `updateEvent cancela el recordatorio viejo antes de programar el nuevo`() = runTest {
        val event = AgendaEventEntity(
            id = "e1", title = "Editado", description = null, dateTimeMillis = 9_000L,
            documentId = null, reminderMinutesBefore = 15, createdAt = 1_000L
        )
    ...
    @Test
    fun `deleteEvent cancela el recordatorio y borra la fila`() = runTest {

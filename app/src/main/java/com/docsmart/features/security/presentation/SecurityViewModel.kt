 ...
 */
data class PendingOriginalDeleteRequest(
...
class SecurityViewModel
    ...
    constructor(
                ...
                try {
                    ...
                    val fileName =
                        com.docsmart.core.util
                            .sanitizeOutputFileName(resolveFileName(context, uri))
        ...
        private fun intentSenderForFailedDelete(
            e: Exception,
            uri: Uri,
        ): IntentSender? {
            val recoverable =
        ...
        fun protectPdfWithPassword(
            ...
            viewModelScope.launch {
                ...
                _uiState.update { state ->
                    when (result) {
                        is PdfPasswordResult.Success ->
                            state.copy(
                                isPdfProcessing = false,
                                pdfOutputFile = result.outputFile,
                                successMessage = result.message,
                            )
                        is PdfPasswordResult.Error ->
                            state.copy(
                                isPdfProcessing = false,
        ...
        fun restoreFile(
            ...
            originalNotDeletedMessage: String,
        ) {
            viewModelScope.launch(Dispatchers.IO) {
                val oldId = file.absolutePath
        ...
        fun dismissOriginalNotDeletedWarning() {
            _uiState.update { it.copy(originalNotDeletedWarning = null) }
        }
        ...
        fun savePdfToDownloads(
            ...
            viewModelScope.launch(Dispatchers.IO) {
                val saved =
                    com.docsmart.core.util.DownloadsSaver
                        .saveFile(context, file, "application/pdf")
                _uiState.update { state ->
                    if (saved) {
                        state.copy(successMessage = String.format(successTemplate, file.name))
                    } else {

...
class ImageFormatUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        try {
            ...
            val bitmapToCompress = if (target.needsWhiteBackground && bitmap.hasAlpha()) {
    ...
    internal fun argbPixelsToBmp(width: Int, height: Int, pixels: IntArray): ByteArray {
    ...
    private fun resolveTargetFormat(targetType: ConversionType): TargetFormat = when (targetType) {
    ...
    private fun writeImage(out: java.io.OutputStream, bitmap: Bitmap, target: TargetFormat) {
        if (target.isBmp) {

...
| Banner de Seguridad con el botón "volver" mal ubicado, reduce el tamaño del banner | Fuera de alcance de este doc — ticket de UI aparte | **Verificado 2026-08-29** en dispositivo: es un patrón deliberado y consistente (el mismo `IconButton` flotante a la izquierda del banner, sin `Scaffold`/`TopAppBar`, ya usado también en `TrashScreen.kt` y otras pantallas secundarias) — no es un defecto aislado de esta pantalla. Corregirlo implicaría rediseñar la convención de navegación compartida por varias pantallas, no un ajuste puntual; queda como parte de "estandarizar banner azul en todas las vistas" (`CONTEXT.md` §2, mejora transversal ya anotada), no resuelto en esta pasada. |
| Falta logo corporativo en el banner de Seguridad | Ticket de UI aparte (estandarización de banners, ya anotado en CONTEXT.md §5) | **No reproducido** (verificado 2026-08-29) — el banner de Seguridad ya muestra el mismo logo "Docu Smart" que el resto de banners de la app; no falta nada respecto a las demás pantallas. |

...
| 16 | El aplanado de anotaciones para "Compartir con anotaciones" no tiene en cuenta la rotación `/Rotate` de la página — en un PDF escaneado con páginas rotadas 90°/270°, el resaltado se ve bien en la app pero puede quedar desplazado en el PDF compartido. Limitación ya documentada en un comentario del propio código, nunca corregida | Baja/Media | `FlattenAnnotationsPdfUseCase.kt:34-45,93-131` | ✅ Corregido (2026-09-16) — nuevo `visualRectToRawPageRect()` en `PdfRectPts.kt` deshace la rotación clockwise de la página antes de dibujar; verificado con PDFs reales rotados 90°/180°/270° en tests unitarios JVM (`FlattenAnnotationsPdfUseCaseTest`), sin necesidad de dispositivo — la evaluación inicial que lo descartó por "no verificable sin dispositivo" era incorrecta |
